...
flow_output_sb_port_binding_handler(struct engine_node *node)
{
     ...
     *    issue for now, since the port-binding related to mac-binding is always
     *    logical router port, and any change to logical router port would just
     *    trigger recompute.
     *
...
#undef OVS_NODE
    ...
    engine_add_input(&en_flow_output, &en_sb_mac_binding, flow_output_sb_mac_binding_handler);
    ...
    while (!exiting) {
        ...
        if (chassis_id) {
            chassis = chassis_run(ovnsb_idl_loop.idl,
        ...
        if (ovnsb_idl_txn && chassis_nb_cfg) {
            ...
            if (cur_cfg && cur_cfg != chassis_nb_cfg->nb_cfg) {
                sbrec_chassis_nb_cfg_set_nb_cfg(chassis_nb_cfg, cur_cfg);
            }
        }
    ...
    if (!restart) {
        ...
        while (!done) {
            ...
            done = chassis_cleanup(ovnsb_idl_loop.idl, ovnsb_idl_txn, chassis) && done;
