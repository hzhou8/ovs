...
main(int argc, char *argv[])
{
    ...
    /* Initialize IDL. */
    idl = the_idl = ovsdb_idl_create(db, &isbrec_idl_class, false, true);
...
/* Casts 'base' into 'struct isbctl_context'. */
static struct isbctl_context *
isbctl_context_cast(struct ctl_context *base)
{
    return CONTAINER_OF(base, struct isbctl_context, base);
}

static void
isbctl_context_invalidate_cache(struct ctl_context *ctx)
{
    struct isbctl_context *isbctl_ctx = isbctl_context_cast(ctx);

    if (!isbctl_ctx->cache_valid) {
        return;
    }
...
isbctl_context_populate_cache(struct ctl_context *ctx)
{
    struct isbctl_context *isbctl_ctx = isbctl_context_cast(ctx);
    ...
    isbctl_ctx->cache_valid = true;
}

OVS_UNUSED static void
pre_get_info(struct ctl_context *ctx)
{
    ovsdb_idl_add_column(ctx->idl, &isbrec_availability_zone_col_name);
}

static struct cmd_show_table cmd_show_tables[] = {
    {&isbrec_table_availability_zone,
     &isbrec_availability_zone_col_name,
     {NULL},
     {NULL, NULL, NULL}},
...
{
    ctl_context_init(&isbctl_ctx->base, command, idl, txn, symtab,
                     isbctl_context_invalidate_cache);
    isbctl_ctx->cache_valid = false;
}
