...
main(int argc, char *argv[])
{
    ...
    /* Initialize IDL. */
    idl = the_idl = ovsdb_idl_create(db, &inbrec_idl_class, false, true);
...
/* Casts 'base' into 'struct inbctl_context'. */
static struct inbctl_context *
inbctl_context_cast(struct ctl_context *base)
{
    return CONTAINER_OF(base, struct inbctl_context, base);
}

static void
inbctl_context_invalidate_cache(struct ctl_context *ctx)
{
    struct inbctl_context *inbctl_ctx = inbctl_context_cast(ctx);

    if (!inbctl_ctx->cache_valid) {
        return;
    }
...
inbctl_context_populate_cache(struct ctl_context *ctx)
{
    struct inbctl_context *inbctl_ctx = inbctl_context_cast(ctx);
    ...
    inbctl_ctx->cache_valid = true;
}

OVS_UNUSED static void
pre_get_info(struct ctl_context *ctx)
{
    ovsdb_idl_add_column(ctx->idl, &inbrec_transit_switch_col_name);
}
...
{
    const struct inbrec_inb_global *inb_global = inbrec_inb_global_first(ctx->idl);
                   ...
                   struct ovsdb_symbol_table *symtab)
{
    ctl_context_init(&inbctl_ctx->base, command, idl, txn, symtab,
                     inbctl_context_invalidate_cache);
    inbctl_ctx->cache_valid = false;
}
